...
namespace CRA.DataMigration.DAL.HealthChecks
{
    public class FirestoreHealthCheck : IHealthCheck
    {
            ...
            CancellationToken cancellationToken = new CancellationToken())
        {
            try
            {
                await _firestoreDb.Collection("Customers").Limit(0).GetSnapshotAsync(cancellationToken);
            ...
            catch (OperationCanceledException ex)
            {
                ...
                _logger.LogWarning(ex,
                    "Health check failed after reaching timeout limit of {TimeoutInMilliseconds} ms",
                    timeoutInMilliseconds);

                var healthCheckResult = new HealthCheckResult(
                    context.Registration.FailureStatus,
                    "Health check failed after reaching timeout limit.");
